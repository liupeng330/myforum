...
import myforum.dao.Page;
import myforum.domain.Board;
import myforum.domain.Post;
import myforum.domain.Topic;
import myforum.domain.User;
import myforum.service.ForumService;
...
public class BoardManageController extends BaseController
{
    ...
    public String addTopic(HttpServletRequest request, Topic topic) {
        ...
        topic.setUser(user);
        Date now = new Date();
        topic.setCreateTime(new Timestamp(now.getTime()));
        topic.setLastPost(new Timestamp(now.getTime()));
